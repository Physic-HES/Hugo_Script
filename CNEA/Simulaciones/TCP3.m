function TCP3(p,sec)
t=1/2880; it=sec/t; m=0.03; u=6*pi*(1E-3)/m*2; g=9.81; time=zeros(it,1); 
Ec=zeros(it-1,1); Ep=zeros(it-1,1); rc=zeros(p,2,p); d=zeros(p,p); radio=0.1/3/2/5;
...
for k=1:it-1;
dx=[pxi(:,k) pyi(:,k)]*([pxi(:,k) pyi(:,k)])'./(ones(p,1)*((pxi(:,k).^2+pyi(:,k).^2).^0.5)')-ones(p,1)*((pxi(:,k).^2+pyi(:,k).^2).^0.5)';
dy=
    for g=1:p;
