...
for j in range(3):
    ...
    plt.pcolormesh(t,f/1000,Sxx,shading='gouraud',norm=LogNorm(vmin=np.max(Sxx)/1E5, vmax=np.max(Sxx)),cmap='jet')
    plt.ylabel(r'Frecuencia [$kHz$]')
    lamb=632E-9
    ind=np.argmax(Sxx,axis=0)
    plt.subplot2grid((3,2),(2,0),rowspan=1)
    plt.plot(t,f[ind]*lamb/2*1000,'.r',label=f'Maximos $\lambda f$')
    ...
    plt.plot(t2,Y,label=r'Ajuste $|vel|$')
    plt.plot(t2,popt1[0]*np.sin(2*np.pi*popt1[1]*t2+popt1[2]),'g',label='Velocidad')
    tiem=datos[j][:,0]-np.min(datos[j][:,0])
    plt.legend()
    plt.subplot2grid((3,2),(0,1),rowspan=1)
    plt.title(arch_d[j])
    plt.plot(tiem,datos[j][:,1]/1000,label='Fotodiodo')
    plt.legend()
    plt.ylabel(r'Voltaje [$mV$]')
    plt.xlim([np.min(tiem),np.max(tiem)])
    plt.grid()
    vel=popt1[0]*np.sin(2*np.pi*popt1[1]*tiem+popt1[2])
    plt.subplot2grid((3,2),(1,1),rowspan=1)
    plt.plot(tiem,vel,'g',label='Velocidad')
    plt.ylabel(r'Velocidad [$mm/s$]')
    plt.xlim([np.min(tiem),np.max(tiem)])
    plt.grid()
    plt.subplot2grid((3,2),(2,1),rowspan=1)
    plt.plot(tiem,1E3*(np.cumsum(vel)*tiem[1]-(np.cumsum(vel)*tiem[1]).mean()),'m',label='Desplazamiento')
