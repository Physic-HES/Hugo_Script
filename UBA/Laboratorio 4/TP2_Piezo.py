...
ax3[0].set_title('Respuesta ante un impulso de 1 Hz con dut ultra corto',fontsize=14)
...
y1=np.fft.fft(S1.values[:,1])
y2=np.fft.fft(S2.values[:,1])
freq=250000*np.linspace(0,1,len(y2))
plt.figure()
plt.loglog(freq[:int(len(freq)/2)]+2.1,np.abs(y1[:int(len(freq)/2)])/len(freq)*2,label='Exitando con cuadrada de fr/2')
plt.loglog(freq[:int(len(freq)/2)]-1.1,np.abs(y2[:int(len(freq)/2)])/len(freq)*2,label='Exitando con cuadrada de fr/3')
plt.annotate(r'$1/3\omega_s$',xy=(16698,0.009),xytext=(16698,0.05),arrowprops=dict(arrowstyle="->"))
plt.annotate(r'$1/2\omega_s$',xy=(25048,0.0001),xytext=(25048,0.002),arrowprops=dict(arrowstyle="->"))
plt.annotate(r'$\omega_s$',xy=(50096,0.3),xytext=(66000,0.1),arrowprops=dict(arrowstyle="->"))
plt.title('Respuesta en frecuencia ante un impulso de 1/2 y 1/3 de la frecuencia de resonancia',fontsize=14)
plt.xlabel('Frecuencia [Hz]',fontsize=14)
plt.xlim([5000,250000/2])
